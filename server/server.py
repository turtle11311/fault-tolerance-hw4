...
from server.authenticate import Authenticator
...
class eVotingServer(voting_pb2_grpc.eVotingServicer, inner_pb2_grpc.eVotingReplicaServicer):
    ...
    def ElectionRecovery(self, request, context):  
        try:
            logging.info("Restoring Backup to other server")
    ...
    def CheckRecovery(self):
        for chan in self.active_channels:
            try:
                status = 1
                inner_stub = inner_pb2_grpc.eVotingReplicaStub(chan[1])
                ...
                if status:
                    logging.info("Recovery from {} success".format(chan[0]))
                else:
                    logging.warning("Recovery from {} fail".format(chan[0]))
    ...
    def serve(self):
        try:
            ...
            self._grpc_server.add_insecure_port(self.bind_addr)
